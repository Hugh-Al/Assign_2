...
// 	Statements
	...
	if (root->data.key == dltKey && root->data.idList.size() > 1) {
//		cout << "Before: " << root->data.idList.size();
//		cout << " '" << root->data.key;
//		cout << " set ---";
////		for(set<int>::iterator i = root->data.idList.begin(); i != root->data.idList.end(); ++i){
////			cout << *i << " ";
////		}
////		cout << " trying to erasse this ";
////		cout << input.getFirstID() << " ";
//		for (std::set<int>::iterator it = root->data.idList.begin();
//				it != root->data.idList.end(); ++it) {
//			cout << *it << ", ";
//		}
//		int todelete = input.getFirstID();
//		root->data.idList.erase(todelete);
//		cout << "' After: " << root->data.idList.size() << " ";
//		cout << "set --- ";
//		for (std::set<int>::iterator i = root->data.idList.begin();
//				i != root->data.idList.end(); ++i) {
//			cout << *i << ", ";
//		}
////		cout << "THE ID " << input.getFirstID() << " and KEY '" << dltKey << "'";
////		cout << " list size now: " << root->data.idList.size() << endl;
		int todelete = input.getFirstID();
		root->data.idList.erase(todelete);
		return root;
	}

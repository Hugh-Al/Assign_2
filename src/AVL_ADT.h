...
private:
	...
	NODE<TYPE> *_delete(NODE<TYPE> *root, KTYPE dltKey, bool& shorter,
			bool& success, TYPE input);

	NODE<TYPE> *dltLeftBalance(NODE<TYPE> *root, bool& smaller);
	NODE<TYPE> *dltRightBalance(NODE<TYPE> *root, bool& shorter);
//	NODE<TYPE> *_delete(NODE<TYPE> *root, KTYPE dltKey, TYPE data,
...
//	NODE<TYPE> *dltRightBalance(NODE<TYPE> *root, bool& shorter);
	...
	void _traversal_postorder(void (*process)(TYPE dataProc), NODE<TYPE> *root);
...
public:
	...
	bool AVL_Delete(KTYPE dltKey, TYPE input);
//	bool AVL_Delete(KTYPE dltKey, TYPE data);
...
template<class TYPE, class KTYPE>
NODE<TYPE>* AvlTree<TYPE, KTYPE>::_insert(NODE<TYPE> *root, NODE<TYPE> *newPtr,
		bool& taller) {
//	Statements
	if (!root) {
		root = newPtr;
...
template<class TYPE, class KTYPE>
bool AvlTree<TYPE, KTYPE>::AVL_Delete(KTYPE dltKey, TYPE input) {
//	Local Definitions
	bool shorter;
	bool success;
...
//	Statements
	newRoot = _delete(tree, dltKey, shorter, success, input);
	cout << "Deleting " << input.key << endl;
	if (success) {
		tree = newRoot;
		count--;
	} // if
...
template<class TYPE, class KTYPE>
NODE<TYPE>* AvlTree<TYPE, KTYPE>::_delete(NODE<TYPE> *root, KTYPE dltKey,
		bool& shorter, bool& success, TYPE input) {
//  Local Definitions
	NODE<TYPE> *dltPtr;
	NODE<TYPE> *exchPtr;
	NODE<TYPE> *newRoot;
...
// 	Statements
	...
	if (dltKey < root->data.key) {
		root->left = _delete(root->left, dltKey, shorter, success, input);
		if (shorter)
			root = dltRightBalance(root, shorter);
	} // if less
	else if (dltKey > root->data.key) {
		root->right = _delete(root->right, dltKey, shorter, success, input);
		if (shorter)
	...
	//  Found equal node
	{
//		cout << "INSIDE INNER LOOP " << input.key << endl;
		if (root->data.key == input.key && root->data.idList.size() > 1) {
			int todelete = input.getFirstID();
			cout << endl << "Key is: " << input.key << " id: " << todelete;
			cout << endl << "Initially list looks like: " << endl;
			for (set<int>::iterator it = root->data.idList.begin();
					it != root->data.idList.end(); ++it) {
				cout << *it << " ";
			}
			root->data.idList.erase(todelete);
			cout << endl << "List now looks like: " << endl;
			for (set<int>::iterator it = root->data.idList.begin();
					it != root->data.idList.end(); ++it) {
				cout << *it << " ";
			}
		...
		//  Delete NODE has two subtrees
		{
			...
			root->left = _delete(root->left, exchPtr->data.key, shorter,
					success, input);
			if (shorter)
				root = dltRightBalance(root, shorter);
		} //  else
...
template<class TYPE, class KTYPE>
NODE<TYPE>* AvlTree<TYPE, KTYPE>::dltLeftBalance(NODE<TYPE> *root,
		bool& smaller) {
//	Local Definitions
	NODE<TYPE> *rightTree;
	NODE<TYPE> *leftTree;
...
template<class TYPE, class KTYPE>
NODE<TYPE>* AvlTree<TYPE, KTYPE>::dltRightBalance(NODE<TYPE> *root,
		bool& shorter) {
//  Local Definitions
	NODE<TYPE> *rightTree;
	NODE<TYPE> *leftTree;
...
template<class TYPE, class KTYPE>
void AvlTree<TYPE, KTYPE>::AVL_Traverse_preorder(
...
template<class TYPE, class KTYPE>
void AvlTree<TYPE, KTYPE>::AVL_Traverse_postorder(
		void (*process)(TYPE dataProc)) {
//	Statements
...
template<class TYPE, class KTYPE>
void AvlTree<TYPE, KTYPE>::_traversal_preorder(void (*process)(TYPE dataproc),
		NODE<TYPE> *root) {
//	Statements
	if (root) {
		process(root->data);
...
template<class TYPE, class KTYPE>
void AvlTree<TYPE, KTYPE>::_traversal_postorder(void (*process)(TYPE dataproc),
		NODE<TYPE> *root) {
//	Statements
	if (root) {
		_traversal_preorder(process, root->left);
...
void AvlTree<TYPE, KTYPE>::_print(NODE<TYPE> *root, int level) {
	...
	if (root) {
		...
		cout << "bal " << setw(3) << root->bal << ": Level " << setw(3)
				<< level;
...
//	//If root already in AVL_tree
//	if (newPtr->data.key == root->data.key) {
//		root->data.idList.push_back(newPtr->data.idList[0]);
//		vector<int>::iterator it;
//		it = unique(root->data.idList.begin(), root->data.idList.end());
//		root->data.idList.resize(distance(root->data.idList.begin(), it));
//		return root;
//	}
//
//	if (newPtr->data.key < root->data.key) {
...
///JACKS
//template<class TYPE, class KTYPE>
//bool AvlTree<TYPE, KTYPE>::AVL_Delete(KTYPE dltKey, TYPE data) {
////	Local Definitions
//	bool shorter;
//	bool success;
//
...
//
////	Statements
//	newRoot = _delete(tree, dltKey, data, shorter, success);
//	if (success) {
//		tree = newRoot;
//		count--;
//	} // if
...
//template<class TYPE, class KTYPE>
//NODE<TYPE>* AvlTree<TYPE, KTYPE>::_delete(NODE<TYPE> *root, KTYPE dltKey,
//		TYPE data, bool& shorter, bool& success) {
////  Local Definitions
//	NODE<TYPE> *dltPtr;
//	NODE<TYPE> *exchPtr;
//	NODE<TYPE> *newRoot;
...
//		return NULL;
//	} //  if -- base case
////	if (root->data.idList.size() > 1 && root->data.key == dltKey) {
////		int search = data.idList[0];
////		int first = 0;
////		int last = root->data.idList.size() - 1;
////		while (first <= last) {
////			int middle = (first + last) / 2;
////			if (root->data.idList[middle] < search) {
////				first = middle + 1;
////			} else if (root->data.idList[middle] == search) {
////				root->data.idList.erase(root->data.idList.begin() + middle);
////				return root;
////			} else {
////				last = middle - 1;
////			}
...
////	}
//	if (dltKey < root->data.key) {
//		root->left = _delete(root->left, dltKey, data, shorter, success);
//		if (shorter)
//			root = dltRightBalance(root, shorter);
//	} // if less
//	else if (dltKey > root->data.key) {
//		root->right = _delete(root->right, dltKey, data, shorter, success);
//		if (shorter)
//			root = dltLeftBalance(root, shorter);
//	} //  if greater
//	else
...
//
//			root->data = exchPtr->data;
//			root->left = _delete(root->left, exchPtr->data.key, data, shorter,
//					success);
//			if (shorter)
//				root = dltRightBalance(root, shorter);
//		} //  else
